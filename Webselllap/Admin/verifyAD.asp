<%
	...
	conn.Open "DRIVER={SQL Server};SERVER=ADMINISTRATOR;UID=sa;PWD=123456;DATABASE=CUA_HANG_MAY_TINH;"
	set rs = server.CreateObject ("ADODB.Recordset")		
	'Open record with entered username
	rs.Open "SELECT * FROM Admin where TaiKhoan='"& Username &"'", conn, 1 
	...
	if rs("MatKhau") = Password then
		Session("name") = rs("TaiKhoan")
		rs.Close
		conn.Close
		set rs=nothing
		set conn=nothing
		Response.Redirect("indexAD.asp")
