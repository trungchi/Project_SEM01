...
<%
Dim DESKTOP_APPLE
Dim DESKTOP_APPLE_cmd
Dim DESKTOP_APPLE_numRows

Set DESKTOP_APPLE_cmd = Server.CreateObject ("ADODB.Command")
DESKTOP_APPLE_cmd.ActiveConnection = MM_Connection_STRING
DESKTOP_APPLE_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and MaLoai = 2   and MaNSX = 5" 
DESKTOP_APPLE_cmd.Prepared = true

Set DESKTOP_APPLE = DESKTOP_APPLE_cmd.Execute
DESKTOP_APPLE_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
DESKTOP_APPLE_numRows = DESKTOP_APPLE_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim DESKTOP_APPLE_total
Dim DESKTOP_APPLE_first
Dim DESKTOP_APPLE_last

' set the record count
DESKTOP_APPLE_total = DESKTOP_APPLE.RecordCount

' set the number of rows displayed on this page
If (DESKTOP_APPLE_numRows < 0) Then
  DESKTOP_APPLE_numRows = DESKTOP_APPLE_total
Elseif (DESKTOP_APPLE_numRows = 0) Then
  DESKTOP_APPLE_numRows = 1
End If

' set the first and last displayed record
DESKTOP_APPLE_first = 1
DESKTOP_APPLE_last  = DESKTOP_APPLE_first + DESKTOP_APPLE_numRows - 1

' if we have the correct record count, check the other stats
If (DESKTOP_APPLE_total <> -1) Then
  If (DESKTOP_APPLE_first > DESKTOP_APPLE_total) Then
    DESKTOP_APPLE_first = DESKTOP_APPLE_total
  End If
  If (DESKTOP_APPLE_last > DESKTOP_APPLE_total) Then
    DESKTOP_APPLE_last = DESKTOP_APPLE_total
  End If
  If (DESKTOP_APPLE_numRows > DESKTOP_APPLE_total) Then
    DESKTOP_APPLE_numRows = DESKTOP_APPLE_total
  End If
End If
%>
...
Set MM_rs    = DESKTOP_APPLE
MM_rsCount   = DESKTOP_APPLE_total
MM_size      = DESKTOP_APPLE_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
DESKTOP_APPLE_first = MM_offset + 1
DESKTOP_APPLE_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (DESKTOP_APPLE_first > MM_rsCount) Then
    DESKTOP_APPLE_first = MM_rsCount
  End If
  If (DESKTOP_APPLE_last > MM_rsCount) Then
    DESKTOP_APPLE_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM MÁY DESKTOP APPLE MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT DESKTOP_APPLE.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(DESKTOP_APPLE.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(DESKTOP_APPLE.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(DESKTOP_APPLE.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(DESKTOP_APPLE.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspDesktop.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(DESKTOP_APPLE.Fields.Item("MaSP").Value)%>">
      <label>
