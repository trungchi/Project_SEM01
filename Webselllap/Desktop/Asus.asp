...
<%
Dim DESKTOP_ASUS
Dim DESKTOP_ASUS_cmd
Dim DESKTOP_ASUS_numRows

Set DESKTOP_ASUS_cmd = Server.CreateObject ("ADODB.Command")
DESKTOP_ASUS_cmd.ActiveConnection = MM_Connection_STRING
DESKTOP_ASUS_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and MaLoai = 2   and MaNSX = 2" 
DESKTOP_ASUS_cmd.Prepared = true

Set DESKTOP_ASUS = DESKTOP_ASUS_cmd.Execute
DESKTOP_ASUS_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
DESKTOP_ASUS_numRows = DESKTOP_ASUS_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim DESKTOP_ASUS_total
Dim DESKTOP_ASUS_first
Dim DESKTOP_ASUS_last

' set the record count
DESKTOP_ASUS_total = DESKTOP_ASUS.RecordCount

' set the number of rows displayed on this page
If (DESKTOP_ASUS_numRows < 0) Then
  DESKTOP_ASUS_numRows = DESKTOP_ASUS_total
Elseif (DESKTOP_ASUS_numRows = 0) Then
  DESKTOP_ASUS_numRows = 1
End If

' set the first and last displayed record
DESKTOP_ASUS_first = 1
DESKTOP_ASUS_last  = DESKTOP_ASUS_first + DESKTOP_ASUS_numRows - 1

' if we have the correct record count, check the other stats
If (DESKTOP_ASUS_total <> -1) Then
  If (DESKTOP_ASUS_first > DESKTOP_ASUS_total) Then
    DESKTOP_ASUS_first = DESKTOP_ASUS_total
  End If
  If (DESKTOP_ASUS_last > DESKTOP_ASUS_total) Then
    DESKTOP_ASUS_last = DESKTOP_ASUS_total
  End If
  If (DESKTOP_ASUS_numRows > DESKTOP_ASUS_total) Then
    DESKTOP_ASUS_numRows = DESKTOP_ASUS_total
  End If
End If
%>
...
Set MM_rs    = DESKTOP_ASUS
MM_rsCount   = DESKTOP_ASUS_total
MM_size      = DESKTOP_ASUS_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
DESKTOP_ASUS_first = MM_offset + 1
DESKTOP_ASUS_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (DESKTOP_ASUS_first > MM_rsCount) Then
    DESKTOP_ASUS_first = MM_rsCount
  End If
  If (DESKTOP_ASUS_last > MM_rsCount) Then
    DESKTOP_ASUS_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM MÁY DESKTOP ASUS MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT DESKTOP_ASUS.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(DESKTOP_ASUS.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(DESKTOP_ASUS.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(DESKTOP_ASUS.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(DESKTOP_ASUS.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspDesktop.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(DESKTOP_ASUS.Fields.Item("MaSP").Value)%>">
      <label>
