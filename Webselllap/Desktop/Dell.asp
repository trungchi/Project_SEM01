...
<%
Dim DESKTOP_DELL
Dim DESKTOP_DELL_cmd
Dim DESKTOP_DELL_numRows

Set DESKTOP_DELL_cmd = Server.CreateObject ("ADODB.Command")
DESKTOP_DELL_cmd.ActiveConnection = MM_Connection_STRING
DESKTOP_DELL_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and MaLoai = 2   and MaNSX = 1" 
DESKTOP_DELL_cmd.Prepared = true

Set DESKTOP_DELL = DESKTOP_DELL_cmd.Execute
DESKTOP_DELL_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
DESKTOP_DELL_numRows = DESKTOP_DELL_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim DESKTOP_DELL_total
Dim DESKTOP_DELL_first
Dim DESKTOP_DELL_last

' set the record count
DESKTOP_DELL_total = DESKTOP_DELL.RecordCount

' set the number of rows displayed on this page
If (DESKTOP_DELL_numRows < 0) Then
  DESKTOP_DELL_numRows = DESKTOP_DELL_total
Elseif (DESKTOP_DELL_numRows = 0) Then
  DESKTOP_DELL_numRows = 1
End If

' set the first and last displayed record
DESKTOP_DELL_first = 1
DESKTOP_DELL_last  = DESKTOP_DELL_first + DESKTOP_DELL_numRows - 1

' if we have the correct record count, check the other stats
If (DESKTOP_DELL_total <> -1) Then
  If (DESKTOP_DELL_first > DESKTOP_DELL_total) Then
    DESKTOP_DELL_first = DESKTOP_DELL_total
  End If
  If (DESKTOP_DELL_last > DESKTOP_DELL_total) Then
    DESKTOP_DELL_last = DESKTOP_DELL_total
  End If
  If (DESKTOP_DELL_numRows > DESKTOP_DELL_total) Then
    DESKTOP_DELL_numRows = DESKTOP_DELL_total
  End If
End If
%>
...
Set MM_rs    = DESKTOP_DELL
MM_rsCount   = DESKTOP_DELL_total
MM_size      = DESKTOP_DELL_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
DESKTOP_DELL_first = MM_offset + 1
DESKTOP_DELL_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (DESKTOP_DELL_first > MM_rsCount) Then
    DESKTOP_DELL_first = MM_rsCount
  End If
  If (DESKTOP_DELL_last > MM_rsCount) Then
    DESKTOP_DELL_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM MÁY DESKTOP DELL MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT DESKTOP_DELL.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(DESKTOP_DELL.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(DESKTOP_DELL.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(DESKTOP_DELL.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(DESKTOP_DELL.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspDesktop.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(DESKTOP_DELL.Fields.Item("MaSP").Value)%>">
      <label>
