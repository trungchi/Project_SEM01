...
Dim DESKTOP_HP
Dim DESKTOP_HP_cmd
Dim DESKTOP_HP_numRows

Set DESKTOP_HP_cmd = Server.CreateObject ("ADODB.Command")
DESKTOP_HP_cmd.ActiveConnection = MM_Connection_STRING
DESKTOP_HP_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and MaLoai = 2   and MaNSX = 4" 
DESKTOP_HP_cmd.Prepared = true

Set DESKTOP_HP = DESKTOP_HP_cmd.Execute
DESKTOP_HP_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
DESKTOP_HP_numRows = DESKTOP_HP_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim DESKTOP_HP_total
Dim DESKTOP_HP_first
Dim DESKTOP_HP_last

' set the record count
DESKTOP_HP_total = DESKTOP_HP.RecordCount

' set the number of rows displayed on this page
If (DESKTOP_HP_numRows < 0) Then
  DESKTOP_HP_numRows = DESKTOP_HP_total
Elseif (DESKTOP_HP_numRows = 0) Then
  DESKTOP_HP_numRows = 1
End If

' set the first and last displayed record
DESKTOP_HP_first = 1
DESKTOP_HP_last  = DESKTOP_HP_first + DESKTOP_HP_numRows - 1

' if we have the correct record count, check the other stats
If (DESKTOP_HP_total <> -1) Then
  If (DESKTOP_HP_first > DESKTOP_HP_total) Then
    DESKTOP_HP_first = DESKTOP_HP_total
  End If
  If (DESKTOP_HP_last > DESKTOP_HP_total) Then
    DESKTOP_HP_last = DESKTOP_HP_total
  End If
  If (DESKTOP_HP_numRows > DESKTOP_HP_total) Then
    DESKTOP_HP_numRows = DESKTOP_HP_total
  End If
End If
%>
...
Set MM_rs    = DESKTOP_HP
MM_rsCount   = DESKTOP_HP_total
MM_size      = DESKTOP_HP_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
DESKTOP_HP_first = MM_offset + 1
DESKTOP_HP_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (DESKTOP_HP_first > MM_rsCount) Then
    DESKTOP_HP_first = MM_rsCount
  End If
  If (DESKTOP_HP_last > MM_rsCount) Then
    DESKTOP_HP_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM MÁY DESKTOP HP MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT DESKTOP_HP.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(DESKTOP_HP.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(DESKTOP_HP.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(DESKTOP_HP.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(DESKTOP_HP.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspDesktop.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(DESKTOP_HP.Fields.Item("MaSP").Value)%>">
      <label>
