...
<%
Dim PHUKIEN_BANPHIM
Dim PHUKIEN_BANPHIM_cmd
Dim PHUKIEN_BANPHIM_numRows

Set PHUKIEN_BANPHIM_cmd = Server.CreateObject ("ADODB.Command")
PHUKIEN_BANPHIM_cmd.ActiveConnection = MM_Connection_STRING
PHUKIEN_BANPHIM_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and TenSP like '%bàn phím%'" 
PHUKIEN_BANPHIM_cmd.Prepared = true

Set PHUKIEN_BANPHIM = PHUKIEN_BANPHIM_cmd.Execute
PHUKIEN_BANPHIM_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
PHUKIEN_BANPHIM_numRows = PHUKIEN_BANPHIM_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim PHUKIEN_BANPHIM_total
Dim PHUKIEN_BANPHIM_first
Dim PHUKIEN_BANPHIM_last

' set the record count
PHUKIEN_BANPHIM_total = PHUKIEN_BANPHIM.RecordCount

' set the number of rows displayed on this page
If (PHUKIEN_BANPHIM_numRows < 0) Then
  PHUKIEN_BANPHIM_numRows = PHUKIEN_BANPHIM_total
Elseif (PHUKIEN_BANPHIM_numRows = 0) Then
  PHUKIEN_BANPHIM_numRows = 1
End If

' set the first and last displayed record
PHUKIEN_BANPHIM_first = 1
PHUKIEN_BANPHIM_last  = PHUKIEN_BANPHIM_first + PHUKIEN_BANPHIM_numRows - 1

' if we have the correct record count, check the other stats
If (PHUKIEN_BANPHIM_total <> -1) Then
  If (PHUKIEN_BANPHIM_first > PHUKIEN_BANPHIM_total) Then
    PHUKIEN_BANPHIM_first = PHUKIEN_BANPHIM_total
  End If
  If (PHUKIEN_BANPHIM_last > PHUKIEN_BANPHIM_total) Then
    PHUKIEN_BANPHIM_last = PHUKIEN_BANPHIM_total
  End If
  If (PHUKIEN_BANPHIM_numRows > PHUKIEN_BANPHIM_total) Then
    PHUKIEN_BANPHIM_numRows = PHUKIEN_BANPHIM_total
  End If
End If
%>
...
Set MM_rs    = PHUKIEN_BANPHIM
MM_rsCount   = PHUKIEN_BANPHIM_total
MM_size      = PHUKIEN_BANPHIM_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
PHUKIEN_BANPHIM_first = MM_offset + 1
PHUKIEN_BANPHIM_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (PHUKIEN_BANPHIM_first > MM_rsCount) Then
    PHUKIEN_BANPHIM_first = MM_rsCount
  End If
  If (PHUKIEN_BANPHIM_last > MM_rsCount) Then
    PHUKIEN_BANPHIM_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM BÀN PHÍMMỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT PHUKIEN_BANPHIM.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(PHUKIEN_BANPHIM.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(PHUKIEN_BANPHIM.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(PHUKIEN_BANPHIM.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(PHUKIEN_BANPHIM.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspPhukien.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(PHUKIEN_BANPHIM.Fields.Item("MaSP").Value)%>">
      <label>
...
<%
PHUKIEN_BANPHIM.Close()
Set PHUKIEN_BANPHIM = Nothing
%>
