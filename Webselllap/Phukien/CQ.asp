...
<%
Dim PHUKIEN_CHUOT
Dim PHUKIEN_CHUOT_cmd
Dim PHUKIEN_CHUOT_numRows

Set PHUKIEN_CHUOT_cmd = Server.CreateObject ("ADODB.Command")
PHUKIEN_CHUOT_cmd.ActiveConnection = MM_Connection_STRING
PHUKIEN_CHUOT_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and TenSP like '%chuột%'" 
PHUKIEN_CHUOT_cmd.Prepared = true

Set PHUKIEN_CHUOT = PHUKIEN_CHUOT_cmd.Execute
PHUKIEN_CHUOT_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
PHUKIEN_CHUOT_numRows = PHUKIEN_CHUOT_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim PHUKIEN_CHUOT_total
Dim PHUKIEN_CHUOT_first
Dim PHUKIEN_CHUOT_last

' set the record count
PHUKIEN_CHUOT_total = PHUKIEN_CHUOT.RecordCount

' set the number of rows displayed on this page
If (PHUKIEN_CHUOT_numRows < 0) Then
  PHUKIEN_CHUOT_numRows = PHUKIEN_CHUOT_total
Elseif (PHUKIEN_CHUOT_numRows = 0) Then
  PHUKIEN_CHUOT_numRows = 1
End If

' set the first and last displayed record
PHUKIEN_CHUOT_first = 1
PHUKIEN_CHUOT_last  = PHUKIEN_CHUOT_first + PHUKIEN_CHUOT_numRows - 1

' if we have the correct record count, check the other stats
If (PHUKIEN_CHUOT_total <> -1) Then
  If (PHUKIEN_CHUOT_first > PHUKIEN_CHUOT_total) Then
    PHUKIEN_CHUOT_first = PHUKIEN_CHUOT_total
  End If
  If (PHUKIEN_CHUOT_last > PHUKIEN_CHUOT_total) Then
    PHUKIEN_CHUOT_last = PHUKIEN_CHUOT_total
  End If
  If (PHUKIEN_CHUOT_numRows > PHUKIEN_CHUOT_total) Then
    PHUKIEN_CHUOT_numRows = PHUKIEN_CHUOT_total
  End If
End If
%>
...
Set MM_rs    = PHUKIEN_CHUOT
MM_rsCount   = PHUKIEN_CHUOT_total
MM_size      = PHUKIEN_CHUOT_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
PHUKIEN_CHUOT_first = MM_offset + 1
PHUKIEN_CHUOT_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (PHUKIEN_CHUOT_first > MM_rsCount) Then
    PHUKIEN_CHUOT_first = MM_rsCount
  End If
  If (PHUKIEN_CHUOT_last > MM_rsCount) Then
    PHUKIEN_CHUOT_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM CHUỘT MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT PHUKIEN_CHUOT.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(PHUKIEN_CHUOT.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(PHUKIEN_CHUOT.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(PHUKIEN_CHUOT.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(PHUKIEN_CHUOT.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspPhukien.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(PHUKIEN_CHUOT.Fields.Item("MaSP").Value)%>">
      <label>
