...
Dim PHUKIEN_USB
Dim PHUKIEN_USB_cmd
Dim PHUKIEN_USB_numRows

Set PHUKIEN_USB_cmd = Server.CreateObject ("ADODB.Command")
PHUKIEN_USB_cmd.ActiveConnection = MM_Connection_STRING
PHUKIEN_USB_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and TenSP like '%usb%'" 
PHUKIEN_USB_cmd.Prepared = true

Set PHUKIEN_USB = PHUKIEN_USB_cmd.Execute
PHUKIEN_USB_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
PHUKIEN_USB_numRows = PHUKIEN_USB_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim PHUKIEN_USB_total
Dim PHUKIEN_USB_first
Dim PHUKIEN_USB_last

' set the record count
PHUKIEN_USB_total = PHUKIEN_USB.RecordCount

' set the number of rows displayed on this page
If (PHUKIEN_USB_numRows < 0) Then
  PHUKIEN_USB_numRows = PHUKIEN_USB_total
Elseif (PHUKIEN_USB_numRows = 0) Then
  PHUKIEN_USB_numRows = 1
End If

' set the first and last displayed record
PHUKIEN_USB_first = 1
PHUKIEN_USB_last  = PHUKIEN_USB_first + PHUKIEN_USB_numRows - 1

' if we have the correct record count, check the other stats
If (PHUKIEN_USB_total <> -1) Then
  If (PHUKIEN_USB_first > PHUKIEN_USB_total) Then
    PHUKIEN_USB_first = PHUKIEN_USB_total
  End If
  If (PHUKIEN_USB_last > PHUKIEN_USB_total) Then
    PHUKIEN_USB_last = PHUKIEN_USB_total
  End If
  If (PHUKIEN_USB_numRows > PHUKIEN_USB_total) Then
    PHUKIEN_USB_numRows = PHUKIEN_USB_total
  End If
End If
%>
...
Set MM_rs    = PHUKIEN_USB
MM_rsCount   = PHUKIEN_USB_total
MM_size      = PHUKIEN_USB_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
PHUKIEN_USB_first = MM_offset + 1
PHUKIEN_USB_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (PHUKIEN_USB_first > MM_rsCount) Then
    PHUKIEN_USB_first = MM_rsCount
  End If
  If (PHUKIEN_USB_last > MM_rsCount) Then
    PHUKIEN_USB_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM PHUKIEN USB MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT PHUKIEN_USB.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(PHUKIEN_USB.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(PHUKIEN_USB.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(PHUKIEN_USB.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(PHUKIEN_USB.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspPhukien.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(PHUKIEN_USB.Fields.Item("MaSP").Value)%>">
      <label>
