...
Dim LAPTOP_DELL
Dim LAPTOP_DELL_cmd
Dim LAPTOP_DELL_numRows

Set LAPTOP_DELL_cmd = Server.CreateObject ("ADODB.Command")
LAPTOP_DELL_cmd.ActiveConnection = MM_Connection_STRING
LAPTOP_DELL_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and MaLoai = 1  and MaNSX = 1" 
LAPTOP_DELL_cmd.Prepared = true

Set LAPTOP_DELL = LAPTOP_DELL_cmd.Execute
LAPTOP_DELL_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
LAPTOP_DELL_numRows = LAPTOP_DELL_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim LAPTOP_DELL_total
Dim LAPTOP_DELL_first
Dim LAPTOP_DELL_last

' set the record count
LAPTOP_DELL_total = LAPTOP_DELL.RecordCount

' set the number of rows displayed on this page
If (LAPTOP_DELL_numRows < 0) Then
  LAPTOP_DELL_numRows = LAPTOP_DELL_total
Elseif (LAPTOP_DELL_numRows = 0) Then
  LAPTOP_DELL_numRows = 1
End If

' set the first and last displayed record
LAPTOP_DELL_first = 1
LAPTOP_DELL_last  = LAPTOP_DELL_first + LAPTOP_DELL_numRows - 1

' if we have the correct record count, check the other stats
If (LAPTOP_DELL_total <> -1) Then
  If (LAPTOP_DELL_first > LAPTOP_DELL_total) Then
    LAPTOP_DELL_first = LAPTOP_DELL_total
  End If
  If (LAPTOP_DELL_last > LAPTOP_DELL_total) Then
    LAPTOP_DELL_last = LAPTOP_DELL_total
  End If
  If (LAPTOP_DELL_numRows > LAPTOP_DELL_total) Then
    LAPTOP_DELL_numRows = LAPTOP_DELL_total
  End If
End If
%>
...
Set MM_rs    = LAPTOP_DELL
MM_rsCount   = LAPTOP_DELL_total
MM_size      = LAPTOP_DELL_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
LAPTOP_DELL_first = MM_offset + 1
LAPTOP_DELL_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (LAPTOP_DELL_first > MM_rsCount) Then
    LAPTOP_DELL_first = MM_rsCount
  End If
  If (LAPTOP_DELL_last > MM_rsCount) Then
    LAPTOP_DELL_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM LAPTOP DELL MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT LAPTOP_DELL.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(LAPTOP_DELL.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(LAPTOP_DELL.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(LAPTOP_DELL.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(LAPTOP_DELL.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspLaptop.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(LAPTOP_DELL.Fields.Item("MaSP").Value)%>">
      <label>
