...
Dim LAPTOP_VAIO
Dim LAPTOP_VAIO_cmd
Dim LAPTOP_VAIO_numRows

Set LAPTOP_VAIO_cmd = Server.CreateObject ("ADODB.Command")
LAPTOP_VAIO_cmd.ActiveConnection = MM_Connection_STRING
LAPTOP_VAIO_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and MaLoai = 1  and MaNSX = 9" 
LAPTOP_VAIO_cmd.Prepared = true

Set LAPTOP_VAIO = LAPTOP_VAIO_cmd.Execute
LAPTOP_VAIO_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
LAPTOP_VAIO_numRows = LAPTOP_VAIO_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim LAPTOP_VAIO_total
Dim LAPTOP_VAIO_first
Dim LAPTOP_VAIO_last

' set the record count
LAPTOP_VAIO_total = LAPTOP_VAIO.RecordCount

' set the number of rows displayed on this page
If (LAPTOP_VAIO_numRows < 0) Then
  LAPTOP_VAIO_numRows = LAPTOP_VAIO_total
Elseif (LAPTOP_VAIO_numRows = 0) Then
  LAPTOP_VAIO_numRows = 1
End If

' set the first and last displayed record
LAPTOP_VAIO_first = 1
LAPTOP_VAIO_last  = LAPTOP_VAIO_first + LAPTOP_VAIO_numRows - 1

' if we have the correct record count, check the other stats
If (LAPTOP_VAIO_total <> -1) Then
  If (LAPTOP_VAIO_first > LAPTOP_VAIO_total) Then
    LAPTOP_VAIO_first = LAPTOP_VAIO_total
  End If
  If (LAPTOP_VAIO_last > LAPTOP_VAIO_total) Then
    LAPTOP_VAIO_last = LAPTOP_VAIO_total
  End If
  If (LAPTOP_VAIO_numRows > LAPTOP_VAIO_total) Then
    LAPTOP_VAIO_numRows = LAPTOP_VAIO_total
  End If
End If
%>
...
Set MM_rs    = LAPTOP_VAIO
MM_rsCount   = LAPTOP_VAIO_total
MM_size      = LAPTOP_VAIO_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
LAPTOP_VAIO_first = MM_offset + 1
LAPTOP_VAIO_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (LAPTOP_VAIO_first > MM_rsCount) Then
    LAPTOP_VAIO_first = MM_rsCount
  End If
  If (LAPTOP_VAIO_last > MM_rsCount) Then
    LAPTOP_VAIO_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM LAPTOP VAIO MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT LAPTOP_VAIO.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(LAPTOP_VAIO.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(LAPTOP_VAIO.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(LAPTOP_VAIO.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(LAPTOP_VAIO.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspLaptop.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(LAPTOP_VAIO.Fields.Item("MaSP").Value)%>">
      <label>
